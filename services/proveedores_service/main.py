...
async def get_proveedor(proveedor_id: int):
    async with pool.acquire() as conn:
        ...
        # Manejo seguro si no hay equipos
        if equipos:
             resultado['estadisticas_compras'] = dict(equipos[0])
        else:
             resultado['estadisticas_compras'] = {"total": 0, "total_comprado": 0}
    ...
    async with pool.acquire() as conn:
        try:
            proveedor_id = await conn.fetchval(
                ...
                proveedor.contacto_telefono, proveedor.sitio_web, proveedor.notas
            )
